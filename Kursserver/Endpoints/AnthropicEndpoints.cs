...
namespace Kursserver.Endpoints
{
    ...
    {
        public static void MapAnthropicEndpoints(this WebApplication app)
        {
            var group = app.MapGroup("api/anthropic").WithTags("AI");


            group.MapPost("exercise-asserts",
                [Authorize] async (ExerciseRequest request, AnthropicService anthropicService, ApplicationDbContext db, HttpContext context) =>
            {
                ...
                try
                {
                    ...
                    var result = await anthropicService.GetCompletionAsync(prompt);
                    var (title, description, example, assumptions, functionSignature, asserts, solution) = ExerciseResponseParser.ParseAssertResponse(result);
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        Success = false,
                        Error = "Failed to generate assertions. Please try again."
                    });
                }
            });

            group.MapPost("project-asserts",
                [Authorize] async (ProjectRequest request, AnthropicService anthropicService, ApplicationDbContext db, HttpContext context) =>
            {
                if (!request.IsValid(out var error))
                {
                ...
                try
                {
                    ...
                    var recentProjects = await db.ProjectHistories
                        .Where(h => h.UserId == userId
                                 && h.TechStack == request.TechStack) // ✅ Only filter by tech stack
                        .OrderByDescending(h => h.CreatedAt)
                        .Take(10)
                        .Select(h => new { h.Title, h.Description })
                        .ToListAsync();
